...
await Bun.write(
	`${dir}${label}.gumbo.json.gz`,
	Bun.gzipSync(new TextEncoder().encode(JSON.stringify(gumbo)))
);

console.log(`fetching savant for ${gamePk}...`);
const savant = await getSavantGameFeed(gamePk);
await Bun.write(
	`${dir}${label}.savant.json.gz`,
	Bun.gzipSync(new TextEncoder().encode(JSON.stringify(savant)))
);
