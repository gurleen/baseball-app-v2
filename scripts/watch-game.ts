...
await new Promise<void>((resolve, reject) => {
	...
	websocket.addEventListener('error', () => reject(new Error(`could not connect to ${url}`)), {
		once: true
	});
});
...
for await (const event of await client.game.subscribe({ gamePk })) {
	...
	switch (event.t) {
		...
		case 'state':
			console.log(
				`STATE     ${event.state.detail} ${event.state.count.balls}-${event.state.count.strikes}, ${event.state.outs} out`
			);
			break;
		case 'linescore':
