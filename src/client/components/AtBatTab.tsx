...
export function AtBatTab({ snapshot }: { snapshot: GameSnapshot }) {
	...
	const seasonMix = play ? (snapshot.seasonPitchMixByPitcher[play.pitcherId] ?? []) : [];
	...
	return (
		<div style={{ ...responsiveColumns(280), gap: 'var(--sp-3)', alignItems: 'start' }}>
			<Panel
				...
				title={
					batter
						? `${batter.shortName} ${batter.batSide ? `(${batter.batSide})` : ''}`.trim()
						: 'BATTER'
				}
				meta={zoneLabel}
				padded={false}
			...
			<Panel
				style={{ ...shrinkable, minWidth: 0 }}
				title="SEQUENCE"
				meta={`${pitches.length} pitches`}
				padded={false}
			>
				{showResult && play ? (
				...
				{mix.length > 0 || seasonMix.length > 0 ? (
					<div
						style={{
					...
					>
						<PitchMixChart today={mix} season={seasonMix} pitcherName={pitcher?.shortName} />
					</div>
				) : null}
			</Panel>
			...
			{side ? (
				<Panel
					style={shrinkable}
					title={snapshot.teams[side].name.toUpperCase()}
					meta="LINEUP"
					padded={false}
				>
					<table style={table}>
							...
							{lineup.length === 0 ? (
								...
								lineup.map(({ slot, playerId, line }) => {
									...
									return (
										<tr
											key={playerId}
											style={current ? { background: 'var(--warn-bg)' } : undefined}
										>
											<td style={{ ...td, ...numeric, fontWeight: 700 }}>{slot}</td>
											<td style={{ ...td, ...copy, fontSize: 'var(--fs-13)' }}>
												{player?.shortName ?? line?.name ?? `#${playerId}`}
											</td>
											<td style={{ ...td, ...numeric, ...muted }}>
