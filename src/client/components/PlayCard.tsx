...
function HitLine({
	...
	if (hit.exitVelocity != null) {
		cells.push(
			<HitCell
				key="ev"
				label="EV"
				value={`${hit.exitVelocity.toFixed(1)} mph`}
				hot={hit.exitVelocity >= 95}
			/>
		);
	}
	...
	return (
		<div
			style={{ display: 'flex', flexWrap: 'wrap', gap: 'var(--sp-2)', marginTop: 'var(--sp-2)' }}
		>
			{cells}
		</div>
	);
...
function HitCell({ label, value, hot }: { label: string; value: string; hot?: boolean }) {
	return (
		...
		>
			<span style={{ background: 'var(--bg-3)', padding: '0 var(--sp-1)', fontWeight: 700 }}>
				{label}
			</span>
			<span style={{ color: hot ? 'var(--warn)' : 'var(--fg-2)', textDecoration: 'underline' }}>
				{value}
			</span>
		</span>
	);
