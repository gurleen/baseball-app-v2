...
import type {
	...
	SchedulePlayer,
	ScheduleTeam
} from '../../server/procedures/schedule.ts';
...
export function GameCard({ game }: { game: ScheduleGame }) {
	...
	return (
		<Link to="/game/$gamePk" params={{ gamePk: String(game.gamePk) }} style={card}>
			...
			>
				<span style={{ ...typeLabel, fontWeight: 700, fontSize: 'var(--fs-12)' }}>
					{statusLabel(game)}
				</span>
			</div>
			...
			{showDecisions && game.decisions ? (
				...
				>
					...
					{game.decisions.save ? (
						<PlayerSlot heading="SAVE" player={game.decisions.save} useGameStats />
					) : null}
				</div>
			) : null}
		</Link>
...
function Matchup({
	...
	return (
		<div
			style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 'var(--sp-3)', minWidth: 0 }}
		>
...
function battingAbbr(game: ScheduleGame): string {
	return game.inning?.half === 'bottom'
