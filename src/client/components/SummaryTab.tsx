		...
		<div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--sp-3)' }}>
			...
			<div style={{ ...responsiveColumns(320), gap: 'var(--sp-3)' }}>
				<Panel
					style={shrinkable}
					title="PLAY BY PLAY"
					meta={`${toPlayLog(snapshot).length}`}
					padded={false}
				>
				...
				>
					{balls.length > 0 ? (
						...
						<div style={{ color: 'var(--fg-3)' }}>
							Statcast has not published batted balls for this game.
						</div>
					)}
				</Panel>
			</div>
...
function GameInfoPanel({ snapshot }: { snapshot: GameSnapshot }) {
	...
	const rows = [
		...
		{
			label: 'Attendance',
			value: info.attendance != null ? info.attendance.toLocaleString() : '—'
		},
			...
			>
				{rows.map((row) => (
					...
					>
						...
						<dd style={{ ...copy, margin: 'var(--sp-1) 0 0', fontSize: 'var(--fs-13)' }}>
							{row.value}
						</dd>
					</div>
				))}
