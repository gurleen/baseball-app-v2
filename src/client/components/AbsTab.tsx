		...
		>
			...
			{rows.length === 0 ? (
				...
				>
					...
					<Panel
						style={{ ...shrinkable, flex: '1 1 20rem' }}
							...
							<table style={{ ...table, minWidth: 560 }}>
								...
								<tbody>
									{rows.map((row) => {
										...
										const direction = formatMissDirection(
											missDirection(row.location, row.strikeZone, row.batterHand)
										);
										return (
											<tr
												key={row.playId}
...
function Count({ value, label, accent }: { value: number; label: string; accent?: string }) {
	return (
		<div style={{ textAlign: 'center' }}>
			<div style={{ ...data, fontSize: 'var(--fs-16)', fontWeight: 800, color: accent }}>
				{value}
			</div>
...
function challengerLabel(snapshot: GameSnapshot, row: AbsChallengeRow): string {
	const player =
		(row.challengerId != null ? snapshot.players[row.challengerId] : undefined) ??
		snapshot.players[row.batterId];
