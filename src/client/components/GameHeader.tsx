...
const scoreGrid: CSSProperties = {
	display: 'grid',
	gridTemplateColumns:
		'auto minmax(1.75rem, 2.75rem) minmax(1.5rem, 2.25rem) minmax(1.5rem, 2.25rem) minmax(1.75rem, 2.75rem) minmax(1.75rem, 2.75rem)',
	alignItems: 'center',
	columnGap: 'var(--sp-2)'
};
		...
		<div style={scoreGrid}>
			<div style={{ display: 'flex', alignItems: 'center', gap: 'var(--sp-3)', minWidth: 0 }}>
				...
				<div style={{ minWidth: 0 }}>
					...
					>
						{showFranchise ? (
							<span style={{ color: 'var(--fg-3)', fontWeight: 500 }}>{franchise}</span>
						) : null}
			...
			<div style={{ ...rhe, fontSize: 'var(--fs-18)', fontWeight: 600 }}>
				{dash(line.moundVisitsRemaining)}
			</div>
			<div style={{ ...rhe, fontSize: 'var(--fs-18)', fontWeight: 600 }}>
				{dash(snapshot.abs?.[side].remaining ?? null)}
			</div>
		</div>
	);
}
	...
	return (
		...
		>
			{state.kind === 'final' ? (
				...
				<span style={{ ...typeLabel, fontWeight: 700, letterSpacing: '0.06em' }}>
					{periodLabel(snapshot) || state.detail.toUpperCase()}
				</span>
			)}
...
function upcomingHitters(
	snapshot: GameSnapshot
): { onDeck: number | null; inHole: number | null } | null {
...
function MatchupStrip({ snapshot }: { snapshot: GameSnapshot }) {
	...
	if (state.kind === 'live') {
		...
		if (play) {
			...
			const pitchingAbbr = offense
				? snapshot.teams[offense === 'home' ? 'away' : 'home'].abbreviation
				: undefined;
			return (
				<div
					style={{
				...
				>
					<div style={matchupGrid}>
						<PitcherSlot
							snapshot={snapshot}
							playerId={play.pitcherId}
							heading={pitchingAbbr ? `PITCHING ${pitchingAbbr}` : 'PITCHING'}
						/>
						<BatterSlot
							snapshot={snapshot}
							playerId={play.batterId}
							heading={battingAbbr ? `BATTING ${battingAbbr}` : 'BATTING'}
						/>
					</div>
...
function DecisionsStrip({ snapshot }: { snapshot: GameSnapshot }) {
	...
	return (
		<div style={matchupGrid}>
			...
			{decisions?.saveId != null ? (
				<PitcherSlot snapshot={snapshot} playerId={decisions.saveId} heading="SAVE" />
			) : null}
		</div>
	);
}
	...
	return (
		<div style={matchupGrid}>
			<PitcherSlot
				snapshot={snapshot}
				playerId={
					snapshot.probablePitchers.away ?? probablePitcherLine(snapshot, 'away')?.playerId ?? null
				}
			...
			<PitcherSlot
				snapshot={snapshot}
				playerId={
					snapshot.probablePitchers.home ?? probablePitcherLine(snapshot, 'home')?.playerId ?? null
				}
...
function PitchCaption({ play }: { play: NonNullable<ReturnType<typeof displayedPlay>> }) {
	...
	const caption = last
		? [
				last.type?.name,
				last.velocity ? `${last.velocity.start.toFixed(1)} mph` : null,
				last.call.name || last.description
			]
				.filter(Boolean)
				.join(' · ')
		: play.description;
	...
	return (
		<div style={{ ...copy, color: 'var(--fg-2)', fontSize: 'var(--fs-13)', lineHeight: 1.4 }}>
			{caption}
		</div>
	);
}
...
function pitcherMeta(line: ReturnType<typeof findPitchingLine>): string | undefined {
	...
	const parts = [
		line.summary,
		line.pitches ? `${line.pitches} PIT (${line.strikes} STR)` : null
	].filter(Boolean);
