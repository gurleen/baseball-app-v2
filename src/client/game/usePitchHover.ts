...
export function usePitchHover(pitches: Pitch[], atBatIndex?: number | null) {
	...
	const zoneIndex =
		focusedNumber == null ? -1 : zonePitches.findIndex((pitch) => pitch.number === focusedNumber);
	const sequenceIndex =
		focusedNumber == null ? -1 : pitches.findIndex((pitch) => pitch.pitchNumber === focusedNumber);
