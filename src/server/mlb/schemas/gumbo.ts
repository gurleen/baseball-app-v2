...
export const PlayEventTypeEnum = z.enum(['pitch', 'pickoff', 'action', 'no_pitch', 'stepoff']);
export type PlayEventType = z.infer<typeof PlayEventTypeEnum>;

export const PositionTypeEnum = z.enum(['Pitcher', 'Catcher', 'Infielder', 'Outfielder', 'Hitter']);
...
export const getPitcherStatsFromGumbo = (
	gumbo: GumboFeed,
	playerId: number
): PitchingStats | null => {
	return (
		gumbo.liveData.boxscore.teams.home.players[`ID${playerId}`]?.stats?.pitching ||
		gumbo.liveData.boxscore.teams.away.players[`ID${playerId}`]?.stats?.pitching ||
		null
	);
};

export const getPitcherSeasonStatsFromGumbo = (
	gumbo: GumboFeed,
	playerId: number
): PitchingStats | null => {
	return (
		gumbo.liveData.boxscore.teams.home.players[`ID${playerId}`]?.seasonStats?.pitching ||
		gumbo.liveData.boxscore.teams.away.players[`ID${playerId}`]?.seasonStats?.pitching ||
		null
	);
};

export const getBatterStatsFromGumbo = (
	gumbo: GumboFeed,
	playerId: number
): BattingStats | null => {
	return (
		gumbo.liveData.boxscore.teams.home.players[`ID${playerId}`]?.stats?.batting ||
		gumbo.liveData.boxscore.teams.away.players[`ID${playerId}`]?.stats?.batting ||
		null
	);
};

export const getBatterSeasonStatsFromGumbo = (
	gumbo: GumboFeed,
	playerId: number
): BattingStats | null => {
	return (
		gumbo.liveData.boxscore.teams.home.players[`ID${playerId}`]?.seasonStats?.batting ||
