...
export const clubGameResults = pgView('club_game_results', {
	...
	sql`SELECT game_pk, club_pk, season, game_date, day_night, double_header, is_home, did_win FROM sqlmesh__public.public__club_game_results__3834347490`
);

export const clubs = pgView('clubs', {
	// You can use { mode: "bigint" } if numbers are exceeding js number limitations
	...
	sql`SELECT pk, name, team_name, club_name, franchise_name, short_name, location_name, abbreviation, team_code, file_code, first_year_of_play, active, sport_pk, league_pk, division_pk, venue_pk FROM sqlmesh__public.public__clubs__1749365355`
);

export const clubsHistory = pgView('clubs_history', {
	// You can use { mode: "bigint" } if numbers are exceeding js number limitations
	pk: bigint({ mode: 'number' }),
	// You can use { mode: "bigint" } if numbers are exceeding js number limitations
	clubPk: bigint('club_pk', { mode: 'number' }),
	...
	sql`SELECT pk, club_pk, season, name, team_name, club_name, franchise_name, short_name, location_name, abbreviation, team_code, file_code, first_year_of_play, active, sport_pk, league_pk, division_pk, venue_pk FROM sqlmesh__public.public__clubs_history__73235740`
);

export const divisions = pgView('divisions', {
	// You can use { mode: "bigint" } if numbers are exceeding js number limitations
	pk: bigint({ mode: 'number' }),
	name: varchar(),
	...
	sql`SELECT pk, name, name_short, abbreviation, league_pk, sport_pk, active FROM sqlmesh__public.public__divisions__3226860093`
);

export const games = pgView('games', {
	// You can use { mode: "bigint" } if numbers are exceeding js number limitations
	pk: bigint({ mode: 'number' }),
	...
	sql`SELECT pk, game_guid, game_type, season, game_date, official_date, day_night, double_header, game_number, scheduled_innings, games_in_series, series_game_number, series_description, venue_pk, home_club_pk, home_score, home_is_winner, away_club_pk, away_score, away_is_winner, abstract_game_state, coded_game_state, detailed_state FROM sqlmesh__public.public__games__3884609178`
);

export const leagues = pgView('leagues', {
	// You can use { mode: "bigint" } if numbers are exceeding js number limitations
	pk: bigint({ mode: 'number' }),
	name: varchar(),
	abbreviation: varchar(),
	...
	sql`SELECT pk, name, abbreviation, name_short, sport_pk, active FROM sqlmesh__public.public__leagues__2544104352`
);

export const people = pgView('people', {
	// You can use { mode: "bigint" } if numbers are exceeding js number limitations
	pk: bigint({ mode: 'number' }),
	...
	sql`SELECT pk, person_id, uuid, retro_id, bbref_id, bbref_minors_id, fangraphs_id, last_name, first_name, given_name, suffix, nickname, birth_date, death_date, mlb_debut_year, mlb_last_season FROM sqlmesh__public.public__people__1739420493`
);

export const pitcherRunsCharged = pgView('pitcher_runs_charged', {
	// You can use { mode: "bigint" } if numbers are exceeding js number limitations
	pk: bigint({ mode: 'number' }),
	...
	sql`SELECT pk, source_id, game_id, play_seq, season, pitcher_pk, is_earned FROM sqlmesh__public.public__pitcher_runs_charged__3179430887`
);

export const playSources = pgView('play_sources', {
...
export const sports = pgView('sports', {
	// You can use { mode: "bigint" } if numbers are exceeding js number limitations
	pk: bigint({ mode: 'number' }),
	code: varchar(),
	name: varchar(),
	...
	sql`SELECT pk, code, name, abbreviation, sort_order, active FROM sqlmesh__public.public__sports__1261206631`
);

export const venues = pgView('venues', {
	// You can use { mode: "bigint" } if numbers are exceeding js number limitations
	pk: bigint({ mode: 'number' }),
	name: varchar(),
	active: boolean(),
	...
	sql`SELECT pk, name, active, address1, address2, city, state, state_abbrev, postal_code, country, phone, elevation, azimuth_angle, latitude, longitude, time_zone_id, time_zone_abbreviation, time_zone_offset, capacity, turf_type, roof_type, field_left_line, field_left, field_left_center, field_center, field_right_center, field_right, field_right_line FROM sqlmesh__public.public__venues__1490727680`
);

export const wobaWeights = pgView('woba_weights', {
	pk: smallint(),
	woba: numeric(),
	...
	sql`SELECT pk, woba, woba_scale, wbb, whbp, w1b, w2b, w3b, whr, run_sb, run_cs, r_pa, r_w, c_fip FROM sqlmesh__public.public__woba_weights__3211996326`
);

export const statcastPitchArsenal = pgView('statcast_pitch_arsenal', {
	// You can use { mode: "bigint" } if numbers are exceeding js number limitations
	pk: bigint({ mode: 'number' }),
	season: smallint(),
	// You can use { mode: "bigint" } if numbers are exceeding js number limitations
...
export const parkFactors = pgView('park_factors', {
	...
	sql`SELECT pk, club_pk, season, basic_5yr, three_yr, one_yr, single, double, triple, hr, so, bb, gb, fb, ld, iffb, fip FROM sqlmesh__public.public__park_factors__2129036003`
);

export const gameDataCompleteness = pgView('game_data_completeness', {
	// You can use { mode: "bigint" } if numbers are exceeding js number limitations
	pk: bigint({ mode: 'number' }),
	season: smallint(),
	gameDate: timestamp('game_date', { withTimezone: true, mode: 'string' }),
	...
	sql`SELECT pk, season, game_date, abstract_game_state, mlb_playbyplay_play_count, has_mlb_playbyplay, statcast_pitch_count, has_statcast_pitches, statcast_batted_ball_count, has_statcast_batted_balls, is_missing_mlb_playbyplay, is_missing_statcast_pitches FROM sqlmesh__public.public__game_data_completeness__2208078641`
);

export const plays = pgView('plays', {
	// You can use { mode: "bigint" } if numbers are exceeding js number limitations
	pk: bigint({ mode: 'number' }),
	sourceId: integer('source_id'),
	gameId: varchar('game_id'),
	...
	sql`SELECT pk, source_id, game_id, play_seq, season, game_date, inning, half_inning, batting_club_pk, pitching_club_pk, batter_pk, pitcher_pk, balls, strikes, outs_after, batter_hand, pitcher_hand, is_single, is_double, is_triple, is_home_run, is_walk, is_intentional_walk, is_hit_by_pitch, is_strikeout, is_sacrifice_fly, is_sacrifice_bunt, is_reached_on_error, is_fielders_choice, is_catcher_interference, is_double_play, is_triple_play, is_other_out, rbi, is_scoring_play, outs_recorded FROM sqlmesh__public.public__plays__3441883269`
);

export const pitchingStatsSeason = pgView('pitching_stats_season', {
	// You can use { mode: "bigint" } if numbers are exceeding js number limitations
	pitcherPk: bigint('pitcher_pk', { mode: 'number' }),
	season: smallint(),
	pa: integer(),
	...
	sql`SELECT pitcher_pk, season, pa, ip, outs, h, singles, doubles, triples, home_runs, bb, ibb, hbp, so, sf, sh, runs, earned_runs, era, whip, k9, bb9, hr9, babip, fip, lob_pct, qualified FROM sqlmesh__public.public__pitching_stats_season__1191742444`
);

export const battingStatsSeason = pgView('batting_stats_season', {
	// You can use { mode: "bigint" } if numbers are exceeding js number limitations
	batterPk: bigint('batter_pk', { mode: 'number' }),
	season: smallint(),
	pa: integer(),
	...
	sql`SELECT batter_pk, season, pa, ab, h, singles, doubles, triples, home_runs, bb, ibb, hbp, so, sf, sh, tb, avg, obp, slg, ops, bb_pct, k_pct, bb_k, iso, babip, woba, wrc_plus, qualified FROM sqlmesh__public.public__batting_stats_season__3606623551`
);

export const statcastPitches = pgView('statcast_pitches', {
	// You can use { mode: "bigint" } if numbers are exceeding js number limitations
	pk: bigint({ mode: 'number' }),
	// You can use { mode: "bigint" } if numbers are exceeding js number limitations
	...
	sql`SELECT pk, play_pk, play_id, game_pk, ab_number, season, inning, half_inning, outs, pitch_number, batter_pk, pitcher_pk, catcher_pk, batter_stand, pitcher_throws, team_batting_id, team_fielding_id, balls_before, strikes_before, balls_after, strikes_after, pitch_type, pitch_name, pitch_call, pitch_description, is_strike_swinging, is_in_zone, event_type, release_speed, plate_speed, extension, spin_rate, horizontal_break, induced_vertical_break, plate_x, plate_z, strike_zone_top, strike_zone_bottom, zone, release_pos_x, release_pos_y, release_pos_z, vx0, vy0, vz0, ax, ay, az, bat_speed, is_bip_out, is_sword, pitcher_pa_number, pitcher_time_thru_order, game_total_pitches, pitcher_total_pitches FROM sqlmesh__public.public__statcast_pitches__40484311`
);

export const seasonDataCompleteness = pgView('season_data_completeness', {
	season: smallint(),
	// You can use { mode: "bigint" } if numbers are exceeding js number limitations
