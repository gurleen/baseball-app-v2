...
function splitSummary(
	splits: StatSplit[] | undefined,
	type: string,
	group: StatGroup
): string | null {
	const split = splits?.find(
		(entry) => entry.type.displayName === type && entry.group.displayName === group
	);
...
function pitchingLine(
	stats:
		...
		| { summary?: string; wins?: number; losses?: number; era?: string | number }
		| null
		| undefined
): string | null {
	...
	if (
		...
		(typeof era === 'string' || typeof era === 'number')
	) {
...
function hittingLine(
	stats:
		| Record<string, unknown>
		| { summary?: string; avg?: string | number; obp?: string | number; slg?: string | number }
		| null
		| undefined
): string | null {
	if (!stats) return null;
...
function toGumboPlayer(
	feed: GumboFeed,
	ref: NamedRef | undefined,
	group: StatGroup
): SchedulePlayer | null {
	if (!ref) return null;
...
export function toScheduleGameFromMlb(game: MlbScheduleGame): ScheduleGame {
	...
	return {
		...
		status: toStatus(
			...
			isWarmup(game.status.detailedState, game.statusFlags)
		),
		teams: {
			away: {
...
export function toScheduleGameFromGumbo(feed: GumboFeed): ScheduleGame {
	...
	return {
		...
		status: toStatus(
			...
			isWarmup(feed.gameData.status.detailedState)
		),
		teams: {
