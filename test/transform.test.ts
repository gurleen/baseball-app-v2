...
describe('pitch transform', () => {
	...
	test('merges Savant batted-ball metrics onto the right pitch', async () => {
		const [gumbo, savant] = await Promise.all([
			loadGumboFixture('final'),
			loadSavantFixture('final')
		]);
	...
	test('swing tracking covers whiffs and fouls, not just contact', async () => {
		...
		expect(tracked.some((pitch) => pitch.call.kind === 'strike' && !pitch.call.isInPlay)).toBe(
			true
		);
...
describe('play actions', () => {
	function allActions(snapshot: GameSnapshot) {
		return [
			...snapshot.plays.flatMap((play) => play.actions),
			...(snapshot.currentPlay?.actions ?? [])
		];
	}
	...
	test('lifts every GUMBO actionIndex entry, including status changes', async () => {
		...
		const expected = gumbo.liveData.plays.allPlays
			...
			.flatMap((play) =>
				play.actionIndex.map((index) => play.playEvents[index]).filter((event) => event != null)
			);
		...
		expect(actions.some((action) => action.description === 'Status Change - In Progress')).toBe(
			true
		);
	});
	...
	test('carries stolen-base-class, substitution, and ejection actions on the parent at-bat', async () => {
		...
		expect(caughtStealing?.actions.map((action) => action.eventType)).toContain(
			'caught_stealing_2b'
		);
		expect(
			caughtStealing?.actions.some((action) => /caught stealing/i.test(action.description))
		).toBe(true);
		...
		expect(pitchingChange?.actions.map((action) => action.eventType)).toContain(
			'pitching_substitution'
		);
		expect(
			pitchingChange?.actions.some((action) => /Pitching Change/i.test(action.description))
		).toBe(true);
	...
	test('carries defensive substitutions from a completed game', async () => {
		const snapshot = await snapshotFor('final');
		const play = snapshot.plays.find((entry) =>
			entry.actions.some((action) => action.eventType === 'defensive_substitution')
		);

		expect(play).toBeDefined();
		expect(play!.actions.some((action) => /Defensive Substitution/i.test(action.description))).toBe(
			true
		);
		expect(
			play!.actions.some((action) => action.playerId != null && action.replacedPlayerId != null)
		).toBe(true);
	});
});
...
describe('pitch mix', () => {
	...
	test('counts match a manual tally from the play log', async () => {
		...
		const mix = snapshot.pitchMixByPitcher[starterId]!;
		expect(mix.reduce((sum, entry) => sum + entry.count, 0)).toBe(
			[...manual.values()].reduce((a, b) => a + b, 0)
		);
...
describe('linescore', () => {
	test('uses the null / "X" convention LineScore renders', async () => {
		...
		const homeSum = home.innings.reduce<number>(
...
describe('diff / reduce round-trip', () => {
	...
	test('updated Statcast ABS miss-by arrives as pitchMetrics', async () => {
		const next = await snapshotFor('live');
		const target = [...iteratePitches(next)].find(
			(pitch) => pitch.metrics?.abs?.edgeDistance != null
		);
		expect(target).toBeDefined();
			...
			plays: next.plays.map((play) => ({
				...
				pitches: play.pitches.map((pitch) =>
					pitch.playId === target!.playId && pitch.metrics?.abs
						? {
								...pitch,
								metrics: { ...pitch.metrics, abs: { ...pitch.metrics.abs, edgeDistance: null } }
							}
						: pitch
				)
			})),
	...
	test('seasonPitchMix delta alone updates the client view', async () => {
		...
		const seasonMix = [
			{ code: 'FF', label: 'Four-seam FB', count: 120, percent: 48, averageSpeed: 96.2 }
		];
		...
		expect(events).toEqual([
			{ t: 'seasonPitchMix', seasonPitchMixByPitcher: withSeason.seasonPitchMixByPitcher }
		]);
