...
const snapshot = toGameSnapshot(
	await loadGumboFixture('final'),
	await loadSavantFixture('final'),
	0
);
...
describe('PitchMix projection', () => {
	...
	test('formats the value column with percent and pitch count', () => {
		const bar = toPitchMixBars([
			{ code: 'FF', label: '4-Seam', count: 51, percent: 57, averageSpeed: 96.198 }
		])[0]!;
	...
	test('formats a missing speed as empty', () => {
		const bar = toPitchMixBars([
			{ code: 'UN', label: 'Unknown', count: 1, percent: 100, averageSpeed: null }
		])[0]!;
...
describe('PlayByPlay projection', () => {
	...
	test('lists nested actions before the plate appearance they belong to', async () => {
		...
		expect(
			log.some(
				(entry) =>
					entry.kind === 'action' && entry.action.description === 'Status Change - Pre-Game'
			)
		).toBe(true);
	});
});
...
describe('lineup and ABS projections', () => {
	test('current lineup follows battingOrder', () => {
		...
		expect(
			lineup.every(
				(row, index) => row.slot === index + 1 || snapshot.boxscore.home.battingOrder.length === 0
			)
		).toBe(true);
	});
